#!/bin/bash                                  

base="/path/to/trees_TAXON/EBG_AICc"
name="$base/Apocynaceae.bearing.genes.txt"
trees="$base/gene-trees_Apocynaceae_bearing_AICc_RAxMLebg.trees"

>"$trees"

while read -r line; do
  [[ -z "$line" ]] && continue
  cat "${base}/${line}.ebg_median_support_prediction.newick" >> "$trees"
done < $name

WASTRAL_EXEC="/path/to/ASTER-MacOS/bin/wastral"

$WASTRAL_EXEC -u 0 \
-o "$base/Apocynaceae.wastral.full.annotated.bearing.AICc.s0.tre" \
-i "$trees" \
2> "$base/Apocynaceae.wastral.full.annotated.bearing.AICc.s0.tre.log"
